...
VariableSolver::VariableSolver(VariableContainer* vcon, const ConstraintPartition& cp)
    : var_ctn_(vcon), constr_pttn_(cp), solver_(FactoryMetaSMT::getNewInstance()) {}

std::vector<std::vector<std::string> > VariableSolver::getContradictions() { return contradictions_; }
