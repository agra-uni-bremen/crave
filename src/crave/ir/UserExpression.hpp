...
typedef typename make_expr<terminal, Constraint_Domain, NodePtr>::type expression;
...
expression value_to_expression(Integer const& i) {
  return boost::proto::make_expr<terminal, Constraint_Domain>(Context::new_value(i));
}
