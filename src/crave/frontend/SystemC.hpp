...
struct bitsize_traits<
    T, typename boost::enable_if<is_sysc_dt<T> >::type> : sc_dt_width<T> {};
...
#define RANDV_SCDT_PRIM_INTERFACE(Typename)        \
 public:                                           \
  ...
  bool next() {                                    \
    static distribution<long long> dist;           \
...
#define RANDV_SCDT_REF_EXPR(Typename)                                  \
  template <int N>                                                     \
  struct ReferenceExpressionImpl<                                      \
      randv<Typename<N> > > : public ReferenceExpression {             \
   public:                                                             \
    ReferenceExpressionImpl(randv<Typename<N> > const& value,          \
    ...
    virtual ReferenceExpression::result_type expr() const {            \
      return new EqualOpr(                                             \
          expr_, new Constant(((Typename<N>)value_).to_uint64(), N,    \
                              crave::is_signed<Typename<N> >::value)); \
    }                                                                  \
                                                                       \
   private:                                                            \
...
#define RANDV_SCDT_BINARY_OPERATOR(Typename, BinOp)                       \
  ...
  template <int N>                                                        \
  sc_dt::int64 operator BinOp(sc_dt::int64 const i,                       \
                              randv<Typename<N> > const& r) {             \
