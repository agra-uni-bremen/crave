...
public: \
  ...
  template<typename T> \
  randv<Typename>& operator=(T i) { this->value = ((Typename) i); return *this; } \
  template<typename T> \
  randv<Typename>& operator=(randv<T> const & i) { this->value = (T) i; return *this; } \
...
#define RANDV_SCDT_BINARY_OPERATOR(Typename, BinOp) \
  ...
  sc_dt::int64 operator BinOp (randv< Typename <N> > const & r, T const & i) { return ((Typename <N>) r) BinOp i; } \
  template<int N> \
  sc_dt::int64 operator BinOp (sc_dt::int64 const i, randv< Typename <N> > const & r) { return i BinOp ((Typename <N>) r); } \
...
#define RANDV_SCDT_VALUE_OPERATORS(Typename) \
  ...
  RANDV_SCDT_BINARY_OPERATOR(Typename, >>) \
  template<int N> \
  sc_dt::int64 operator ~ (randv< Typename <N> > const & r) { return ~((Typename <N>) r); } \
