...
rand_obj_gen::rand_obj_gen(unsigned int num, rand_obj_base* obj)
    : number_(num), obj_(obj),
      solutions_(),
      elapsed_gen_times_(),
      values_generated_(false) {
...
void rand_obj_gen::print_bench_values() {
  ...
  for (unsigned int j = 0; j < number_; ++j) {
    ...
    elapsed_min = (elapsed_min < elapsed_gen_times_[j]) ?
    elapsed_min : elapsed_gen_times_[j];
    elapsed_max = (elapsed_max > elapsed_gen_times_[j]) ?
    elapsed_max : elapsed_gen_times_[j];
  ...
  std::cout << "Number of Variables:\t\t "
            << solutions_[0].size() << std::endl;
  std::cout << "Bitwise Hamming Distance (avg):\t "
            << sum / ((number_ - 1) * number_ / 2.)
            << std::endl;
  std::cout << "Bitwise Hamming Distance (min):\t "
            << min << std::endl;
  std::cout << "Bitwise Hamming Distance (max):\t "
            << max << std::endl;
  std::cout << "Wordwise Hamming Distance (avg): "
            << word_sum / ((number_ - 1) * number_ / 2.) << std::endl;
  std::cout << "Wordwise Hamming Distance (min): "
            << word_min << std::endl;
  std::cout << "Wordwise Hamming Distance (max): "
            << word_max << std::endl;
  std::cout << "Elapsed generation time (min):\t "
            << elapsed_min /  static_cast<double>(CLOCKS_PER_SEC) << std::endl;
  std::cout << "Elapsed generation time (max):\t "
            << elapsed_max /  static_cast<double>(CLOCKS_PER_SEC) << std::endl;
  std::cout << "Elapsed generation time (sum):\t "
            << elapsed_gen_time /  static_cast<double>(CLOCKS_PER_SEC)
            << std::endl;
}
