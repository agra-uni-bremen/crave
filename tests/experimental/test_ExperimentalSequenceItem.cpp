...
BOOST_AUTO_TEST_CASE(enum_gt) {
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    BOOST_REQUIRE(obj.player == football_enum::LW || obj.player == football_enum::SS ||
                  obj.player == football_enum::RW || obj.player == football_enum::CF);
  }
}
...
class obj1 : public obj {
 public:
  crv_constraint constraint{dist(e(), distribution<char>::simple_range('A', 'Z')),
...
struct Item2 : public crv_sequence_item {
  ...
  crv_constraint constraint = {
      address() % 4 == 0,                             address() <= 1000u,
      data().size() == 4,                             foreach (data(), -50 <= data()[_i] && data()[_i] <= 50),
